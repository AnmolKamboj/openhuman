...
- **"I don't have that stored" on a personal question while the fact is in memory (issue #6040)** — the turn injects no memory of the user by itself, and the `MemoryAccessSection` instruction was gated on `learning.enabled` (default `false`), so the model was never told to look before claiming absence. Now: Lane C (`src/openhuman/memory/auto_recall/`) runs a lexically gated, bounded `fast_retrieve` through the session binding's guard and prepends a `## Relevant memory for this message` block to the **user message** (never the system prompt — KV-cache prefix); the instruction is added whenever a retrieval tool is registered, regardless of learning; `memory_recall` no longer requires `namespace`. Switch: `[subsystems.memory.hooks] auto_recall` / `OPENHUMAN_MEMORY_HOOKS_AUTO_RECALL` — the flag existed before and was read by nothing. `core_turn.rs` sits near the 750-line gate: per-turn memory lanes live in `turn/recall_lanes.rs`, add there. `RecordingProvider::with_fast_retrieve_result` scripts the guarded retrieval family in tests. The sub-agent fast path (#4677) still skips entity-less queries; Lane C calls the engine directly, whose empty-entity path is dense retrieval. **Latency:** every `fast_retrieve` asks the host for spaCy over the bus (`memory_host.rs::extract_spacy` → the Python server) plus a query embed, so a warm lookup is ~1.5–3 s and the first after boot was >5 s (Python server + model load) — `memory::auto_recall::warm::spawn_at_boot`, hooked in `modules/boot.rs` right after the TinyMemory module loads, runs one throwaway retrieval so the user's first question is warm (measured: warm-up 2.4 s at boot, first real lookup 1.6 s). Lane budget is 5 s; `[memory_host] embed/extract_spacy elapsed_ms` debug lines give the split.
- **A `Memory` trait method left on its default is a silent no-op in production (issue #6041)** — the session's `Arc<dyn Memory>` is `agent/experience/ops.rs::DriverMemory` (since `7a4278378`), and `tinymemory-api`'s `Memory::recall_relevant_by_vector` defaults to `Ok(vec![])`, so situational preferences (Lane B) injected nothing on every module-backed install while every turn test stayed green — the harness tests bind the embedded `tinymemory_core::store::create_memory`, which overrides it. `DriverMemory` now answers through `preferences::recall_by_vector_over` (one body with the guard path). Rule: a turn-level test that pins production behaviour must bind `DriverMemory` over `guard::test_support::RecordingProvider` (`with_namespace_hits`, `with_list_result`), not the embedded store; and before leaving a trait default in place, grep who actually holds the handle. Costs measured on the desktop once the lane was live: the vector recall is one query-embed round trip, 0.8–3 s per turn, so `recall_situational_preferences_on` lists the namespace first (no embed) and skips the recall when nothing is stored; Lane B's budget is 5 s (3 s lost the block on both on-topic turns). Floor tuning data: `[pref_recall] namespace= kept= top_similarity= floor= elapsed_ms=` — with the managed embedder, unrelated message↔preference pairs score 0.28–0.33, a rule-phrased travel preference vs a trip-planning message 0.349, a coffee preference vs a café question 0.575; the 0.35 floor is thin but lowering it pulls cross-topic preferences in.
- **`memory_store` writes land where the pre-turn lane never looked (issue #6063)** — `memory_store` files a fact as a namespace document in `global` (`memory_docs` + a `vector_chunks` embedding, the same path `save_preference` takes), while Lane C read only the memory tree (`fast_retrieve`), which namespace documents never enter; the archivist's chat→tree ingest runs at segment close (days late on a live profile) and short turns stay `buffered`/`dropped`, so "remember X" → new chat → "X is not on record" reproduced on a default install with the #6052 write in place. Now Lane C runs two legs side by side (`tokio::join!`, a 5 s bound **per leg**): the tree walk and `recall_namespace_scored("global", message, 3)` through the same guard (`AutoRecallSource::recall_namespace_scored` — required, no trait default, per the #6041 rule), floored at `AUTO_RECALL_NOTE_MIN_SIMILARITY = 0.35` on the vector component (Lane B's floor data), notes rendered first as `- <content> (note: <key>)`, wrapped when `ExternalSync`-tainted or connector-keyed. The block also opens with `AUTO_RECALL_HINT` ("pre-fetched, not a search") because the model read the old block as the retrieval the memory-access rule demands and claimed absence over three unrelated tree hits. Tell in the log: `[auto_recall] … hits= total= notes= notes_top= tree_ms= notes_ms=` — a `total` unchanged after a store means the fact never reached the tree. `RecordingProvider::with_namespace_hits` + `guard::test_support::namespace_hit` script the leg; `guarded_with` + `AutoRecall::from_guard` is the production wiring in a turn test.
- **Composio `list_connections` at boot while signed out reported ERR + Sentry (issue #6176)** — in backend mode with no app-session JWT, `modules::connectors::module_config` fails, `ensure_routed` sends the connector module `{"route": "none"}`, and every routed member answers "this module was loaded without a connector route … or call Configure" (Configure _was_ called; the text is misleading). `start_bootstrap_jobs` → `memory::sources::reconcile::ensure_composio_sources` → `composio_list_connections` hit it at every boot (the reconcile `debug!`-swallows the failure one layer up, after `report_composio_op_error` had already fired), and `start_periodic_sync` repeated it every 60 s. Fix: `integrations/composio/ops/error_utils.rs::backend_mode_without_session`, the twin of `direct_mode_without_key`, short-circuits `composio_list_connections` with the quiet `COMPOSIO_NO_SESSION` `Err` ("no backend session token", the wording `resolve_client` already uses, which `is_session_expired_message` demotes at the JSON-RPC boundary) and skips `report_composio_op_error` — NOT an `Ok(empty)`: Codex (PR#6178) pointed out that an empty success reads as authoritative to `memory::sources::reconcile` (`list_rpc` would hide every persisted Composio row) and to `flows::validate_connection_refs` (every `connection_ref` false-rejected), while `Err` keeps their documented fail-open path; direct-mode-without-key returns `Ok(empty)` only because no key ⇒ no tenant ⇒ truly no connections. It reads `api::jwt::get_session_token` — the single token source `integrations::build_client` uses — and not `build_client` itself, which WARNs on every call; a _failed_ lookup returns `false` so a real store fault still surfaces. Also: a corrupt `auth-profiles.json` is quarantined + reset rather than an error, so to test the `Err` branch make `config_path.parent()` a regular file (the `auth_profile_lock_errors_do_not_include_local_paths` trick). `ops_tests_part_03_tests.rs` sits at 720 of the 750-line layout gate — new Composio op tests go in `ops_tests_part_04_tests.rs`.
...
## Sources row: pending vs stuck vectors (Issue #6025)

- **`chunks_pending` alone is ambiguous** — since tinymemory 1.14 vectors come from the `reembed_backfill` job, not inline at ingest, so every sizeable sync leaves a transient backlog. `sourcePipelineStatus.ts` reads it as soft `vectors_pending` while `memory_tree_memory_backfill_status.in_progress` is true or `freshness` is `active`/`recent` (≤5 min); amber `stored_without_vectors` only for the `semantic_recall` latch, an embeddings-family `first_blocking_cause`, or an idle backlog with nothing queued.
- **`backfill_in_progress` is a process-global flag, not a query** — set when a re-embed chain is armed, cleared only when the chain ends (Done/Skipped/StaleSignature/Failed), so it stays true while the job sits starved in `ready` behind `extract_chunk` on the single LLM permit. Not per-workspace.
- **Test fixtures: a stuck backlog needs `freshness: 'idle'`** — `chunks_pending > 0` with `recent`/`active` freshness renders the neutral note, not the warning. `vi.mock` factories for `utils/tauriCommands/memoryTree` must list `memoryTreeBackfillStatus` or `MemorySourcesRegistry.refresh` throws inside its `Promise.all`.
- **`t()` does not interpolate** — the caller replaces placeholders: `t('sync.pipeline.vectorsPending').replace('{count}', n.toLocaleString())` (pattern from `SyncAuditPanel.tsx`).
- **`SYNC_PASS_MAX_ITEMS` (200) sizes one `AcceptSourceItems` call** — the module embeds each document in turn (~1.7 s each) against the 15-min `module_call_slow!` deadline; 500 tripped it. The Sources-row loop (`composio_sync_budgeted`, ≤50 passes) is unaffected; the four entry points that sync once per invocation (`periodic.rs`, `memory/ops/sync.rs`, `bus_part_02.rs` `connection_created`, `slack/rpc.rs`) go through `ops::pass_budget::run_sync_within_budget`, which repeats the pass within a 500-**record** budget. Charge that budget by `records_read`, never by `written`: a module returning already-ingested pages with `more_pending` would otherwise be asked forever.
- **`providers_ops.rs` sits at exactly 750 lines** — the `check-openhuman-rust-layout.mjs` limit; edits there must be net-zero lines or split the file.
- **`node app/scripts/build-parallel.mjs` fails with `vite: command not found`** when run directly — it shells out to bare `tsc`/`vite`; use `pnpm build` or prefix `PATH=app/node_modules/.bin:$PATH`.
- **"Got it, saved" with zero tool calls (issue #6048)** — the memory *write* tools (`memory_store`, `save_preference`) are always registered, but nothing in the prompt said a request to remember must become a tool call, or forbade claiming a save without one; `memory_store` also required an invented `namespace` + `key`. Now `MemoryWriteSection` (`agent/learning/prompt_sections.rs`, `## Remembering`) is registered by `builder/helpers.rs::add_memory_prompt_sections` whenever a write tool is registered **and** visible, independent of `learning.enabled`; `memory_store` defaults `namespace` to `DEFAULT_AGENT_MEMORY_NAMESPACE` — the assistant's own scope, currently `"global"`, the same namespace the rest of recall reads — and derives `key` from the content (`derive_key`: a readable stem plus a 16-hex SHA-256 suffix; 64 bits, because the stem collides by design and a key collision *overwrites* an unrelated memory rather than duplicating it). The section names only the write tools the session actually offers: `archivist`, `trigger_reactor` and `workflow_builder` carry `memory_store` without `save_preference`, so naming both unconditionally would mandate an invisible tool. The archivist is not a safety net here: it writes the turn to the FTS5 episodic table only, and files raw prose into the tree at segment close. `factory.rs` sits at its legacy cap (1552): put new gating in `helpers.rs`, not inline.
