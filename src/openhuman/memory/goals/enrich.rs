...
fn build_prompt(context_input: &str, first_run: bool) -> String {
    let mode = if first_run {
        "The goals list is currently EMPTY. This is the first run — populate \
         ...
         the context below. Start with `goals { "op": "list" }` to confirm, \
         then `op: "add"` for each goal."
    } else {
        "Maintain the existing goals list. Call `goals { \"op\": \"list\" }` \
         first, then make the MINIMAL set of changes (add / edit / delete) \
